...
export function useMatchesCached() {
  ...
  const [matches, setMatches] = useState(() =>
    sortByStartTime(initialCache.cachedFinished)
  );

  const [loading, setLoading] = useState(true);

  const finishedRef = useRef(initialCache.cachedFinished);
  const nonFinishedRef = useRef([]);
  const metaReadyRef = useRef(false);
  const nonFinishedReadyRef = useRef(false);
  ...
  useEffect(() => {
    ...
    const publishMatches = () => {
      setMatches(
        mergeById(
          finishedRef.current,
          nonFinishedRef.current
        )
      );

      if (metaReadyRef.current && nonFinishedReadyRef.current) {
        setLoading(false);
      }
    };
    ...
    const unsubscribeNonFinished = onSnapshot(
      nonFinishedQuery,
      (snapshot) => {
        nonFinishedRef.current = snapshot.docs.map((document) =>
          deserializeMatch({
            id: document.id,
            ...document.data(),
          })
        ...
        nonFinishedReadyRef.current = true;
        publishMatches();
      },
      (error) => {
    ...
    const unsubscribeMeta = onSnapshot(
      ...
      async (snapshot) => {
        ...
        try {
          const finishedMatches = await resolveFinishedMatches(meta);

          if (cancelled || requestId !== metaRequestIdRef.current) return;

          finishedRef.current = finishedMatches;
